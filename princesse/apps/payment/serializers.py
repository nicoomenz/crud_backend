...
class PaymentSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        if 'status' not in validated_data or not validated_data['status']:
            validated_data['status'] = Payment.STATUS_CHOICES[0][0]
        ...
        for producto_data in productos_data:
            ...
            marca = marca = Marca.objects.get_or_create(id=marca_data.id, defaults={'nombre': marca_data.nombre})[0] if marca_data else None
        ...
        for combo in combo_data:
            combo_instance = Combo.objects.get(**combo)[0]
    ...
    #         variantes = {}
    #         for payment_product in PaymentProduct.objects.filter(payment=instance, producto=producto):
    #             color = ColorSerializer(producto.color).data
    #             talle_data = {
    #                 'talle': TalleSerializer(producto.talle).data,
    #                 'cantidad': payment_product.cantidad
    #             }
    #             if color['id'] not in variantes:
    #                 variantes[color['id']] = {
    #                     'color': color,
    #                     'talles': []
    #                 }
    #             variantes[color['id']]['talles'].append(talle_data)
